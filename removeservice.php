    ...
    if($_SESSION['adminloggedin'])
	{
		...
		if (isset($_POST['removeservice'])) {
			...
			if($conn){
				...
				$stmt->bindparam(':id',$_POST['serviceid']);
				$result=$stmt->execute();
...
<html lang="en">
 ...
 <div class="container">
        <div class="card card-container">
            <p id="profile-name" class="profile-name-card"> Select Service to remove </p>
            <?php echo $error; ?>
            <form class="form-signin" action="removeservice.php" method="post">
            	<?php
			...
			if($conn){
				...
				$stmt->bindparam(':id',$_POST['services']);
				$result=$stmt->execute();
			...
			if(!$result)
			{
				$error="something went wrong!!";
					...
					print "
					<select name='serviceid' style='margin-bottom:1cm'>";
					print "<option value=".$row['id'].">".$row['servicename']."</option>";
					print "</select>
				...
				<select name='services' style='margin-bottom:1cm'>";
					
						
						$conn=connectdatabase();
						$stmt=$conn->prepare("SELECT id,servicename FROM services");
						$stmt->execute();
						$stmt->setFetchMode(PDO::FETCH_ASSOC);
				...
				<div class='form-group'>
				<a href='#'><button class='btn btn-default' name='selectservice'>Select Service</button></a>
				</div>";
				}
				?>
            </form><!-- /form -->
