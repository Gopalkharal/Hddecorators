...
<?php
	...
	if (isset($_POST['addabout'])) {
			...
			if( empty($_POST['date']) || empty($_POST['description']) || empty($_POST['shortdescription']) ){
				$error="About not Added. <br />All fields are mandatory.";
			}
		...
		include	'functions.php';  //connects to mysql server
			...
			if($conn){
				$stmt=$conn->prepare("SELECT MAX(id) FROM about");
				$stmt->execute();
				...
				$destination="img/about/about" . $newfilename;
				move_uploaded_file($_FILES["picture"]["tmp_name"],$destination);
				$stmt=$conn->prepare("INSERT INTO about (time, shortdescription, description, pathtoimage) VALUES (:time, :shortdescription, :description, :pathtoimage)");
				$stmt->bindparam(':time',$_POST['date']);
...
<html lang="en">
 ...
 <div class="container">
        <div class="card card-container">
            ...
            <?php echo $error; ?>
            <form class="form-signin" action="addabout.php" method="post" enctype="multipart/form-data">
